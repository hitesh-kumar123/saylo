...
export const JitsiVideo: React.FC<JitsiVideoProps> = ({
  ...
  useEffect(() => {
    ...
    const initializeVideo = async () => {
      ...
      if (initialized || !videoContainerRef.current) {
        return;
      }
      ...
      try {
        ...
        try {
          ...
          // Stop the stream - Jitsi will create its own
          stream.getTracks().forEach(track => track.stop());
        } catch (permissionError) {
          console.warn("Permission denied, continuing anyway:", permissionError);
        }
        ...
        // Set container for Jitsi
        const container = videoContainerRef.current;
        container.innerHTML = ""; // Clear any previous content

        // Join the room - service will handle initialization with container
        const api = await jitsiService.joinRoom(finalRoomName, userInfo, container);
        ...
        setTimeout(() => {
          if (mounted && isLoading && !isConnected) {
            console.warn("Video connection timeout, showing as connected");
            setIsLoading(false);
            setIsConnected(true);
            if (onConnected) onConnected();
          }
  ...
  return (
    <div className="w-full h-full relative">
      ...
      <div
        ...
        id="jitsi-container"
        className="w-full h-full bg-gray-900 min-h-[200px]"
