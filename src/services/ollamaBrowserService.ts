// Browser-compatible Ollama service using fetch API
...
export class OllamaBrowserService {
  ...
  private async generateText(prompt: string, options?: any): Promise<string> {
    try {
  ...
  async analyzeResponse(
    ...
    try {
      const aiResponse = await this.generateText(prompt, { temperature: 0.3 });
  ...
  async generateInterviewSummary(
    ...
    try {
      const aiResponse = await this.generateText(prompt, { temperature: 0.4 });
      return aiResponse.trim();
    } catch (error) {
...
export const ollamaBrowserService = new OllamaBrowserService();
